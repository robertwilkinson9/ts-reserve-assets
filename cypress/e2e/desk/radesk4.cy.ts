...
*/
  ...
  it('check details about gf08 are correct', () => {
    cy.request({ url: 'https://localhost:6179/api/all_desks', method: 'GET', failOnStatusCode: false}).as('desks');
    cy.get('@desks').its('status').should('eq', 200);
    cy.get('@desks').then((response) => {
      const gf08_info = response.body['data'][1];

      const em_email = gf08_info['email'];
      expect(em_email).to.equal('b@b.b');

      const em_desk = gf08_info['desk'];
      expect(em_desk).to.equal('gf08');

      const em_floor = gf08_info['floor'];
      expect(em_floor).to.equal('ground');
    })
  })
});
